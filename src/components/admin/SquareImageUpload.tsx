...
export default function SquareImageUpload({ value, onChange }: Props) {
  ...
  async function onFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      onChange(data.url, data.storage_path);
    } catch (err) {
    ...
    <div className="grid gap-3">
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold text-brand-navy/55">
          Displayed as a square on the card — any image works, it&apos;ll be auto-cropped to fit.
        </p>
        {value && (
          <button
            type="button"
      ...
      {value && (
        <div className="relative aspect-square w-32 overflow-hidden rounded-xl bg-brand-teal-light">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={value} alt="Card preview" className="h-full w-full object-cover object-center" />
        </div>
      )}
      ...
      <label className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-brand-purple-deep/20 bg-brand-lavender/20 py-6 text-center transition hover:border-brand-purple-bright hover:bg-brand-lavender/40">
        <span className="text-sm font-semibold text-brand-navy/60">
          {uploading ? "Uploading…" : value ? "Replace image" : "Upload card image"}
        </span>
        <span className="text-xs text-brand-navy/40">JPG, PNG, or WebP — under 5MB</span>
        <input
          ref={inputRef}
          type="file"
